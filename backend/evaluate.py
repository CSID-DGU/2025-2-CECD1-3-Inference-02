...
DB에서 실제 대화를 읽어서 GPT-5.4-mini가 평가
"""
        ...
        if log["role"] == "user":
            ...
            if i + 1 < len(conv_logs) and conv_logs[i + 1]["role"] == "assistant":
                pairs.append({
                    ...
                    "prev_context": list(prev_context),  # 이전 대화 복사
                })
...
def main():
    print("=" * 60)
    print("  마음돌봄 챗봇 응답 품질 평가 (LLM-as-a-Judge)")
    print("=" * 60)
    ...
    print(f"\n📋 총 {len(conversations)}개 날짜의 대화 발견")
    for date, logs in conversations.items():
        user_count = sum(1 for l in logs if l["role"] == "user")
        print(f"  {date}: {user_count}건의 사용자 메시지")
    ...
    print(f"\n🔍 평가 대상: 총 {len(all_pairs)}개 대화 쌍")

    print(f"   전체 대화를 평가합니다")
    ...
    for i, pair in enumerate(all_pairs):
        print(f"\n{'─'*50}")
        print(f"평가 {i+1}/{len(all_pairs)} | {pair['date']} | {pair['round_num']}회차")
        ...
        try:
            ...
            scores = " | ".join([
                f"{cat_names[c]}:{result[c]['score']}" for c in cat_names
            ])
    ...
    for cat, name in cat_names.items():
        ...
        print(f"  {name:　<6} {bar} {avg:.1f} / 5.0")

    total = sum(r["total_avg"] for r in results) / len(results)
    print(f"\n  {'종합 평균':　<6} {'━'*20} {total:.1f} / 5.0")

    # 최고/최저 응답
    best = max(results, key=lambda r: r["total_avg"])
    worst = min(results, key=lambda r: r["total_avg"])

    print(f"\n  ✅ 최고 평점 ({best['total_avg']}):")
    print(f"     사용자: {best['user_msg'][:60]}")
    print(f"     챗봇:   {best['bot_reply'][:60]}")

    print(f"\n  ⚠️ 최저 평점 ({worst['total_avg']}):")
    print(f"     사용자: {worst['user_msg'][:60]}")
    print(f"     챗봇:   {worst['bot_reply'][:60]}")
    print(f"     코멘트: {worst.get('overall_comment', '')[:80]}")
